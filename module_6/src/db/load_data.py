...
def create_ingestion_watermarks_table(connection):
    """Create the watermark table used by incremental worker ingestion."""
    create_watermarks_query = """
    ...
    """
    try:
        connection.execute(create_watermarks_query)
        connection.commit()
        print("Ingestion watermarks table created successfully")
    except Exception as e:
        print(f"Error creating ingestion_watermarks table: {e}")
        connection.rollback()
        raise
...
def main():
    ...
    try:
        ...
        create_ingestion_watermarks_table(conn)
